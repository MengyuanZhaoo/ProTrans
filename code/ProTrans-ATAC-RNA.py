...
from encode import encode_gene
...
class ProTrans(nn.Module):
    def __init__(self, head=10, head_dim=100):
        ...
        self.query_linear = nn.Linear(10100, 1000)
...
if __name__ == '__main__':
    # Parse arguments
    parser = argparse.ArgumentParser(description='ProTrans-ATAC-RNA.py')
    ...
    parser.add_argument('--out_dir', type=str, default='../result/GSE64378')
    ...
    criterion_cosin = nn.CosineEmbeddingLoss()

    # Load ATAC Data
    atac = pd.read_csv(args.data_dir+'/ATAC-RNA/atac.csv', sep = ',', header=0, index_col= 0 )  # (cell, peak)
    mrna = pd.read_csv(args.data_dir+'/ATAC-RNA/rna.csv', sep = ',', header=0, index_col= 0 )  # (cell, gene)
    
    # filter peaks with chr
    atac = atac[[col for col in atac.columns if col.startswith('chr')]]
    # align cell number
    cells = np.intersect1d(np.array(atac.index),np.array(mrna.index))
    atac = atac.loc[cells,:]
    mrna = mrna.loc[cells,:]
    ...
    # Load RNA Data
    print('Raw mRNA data shape:'+str(mrna.shape))
    obs = pd.DataFrame({'cell':np.array(mrna.index)})
    var = pd.DataFrame({'gene':np.array(mrna.columns)})
    adata_mrna = ad.AnnData(X=mrna.values, obs=obs, var=var, dtype=np.float32)
    sc.pp.filter_genes(adata_mrna, min_cells = 10)
    sc.pp.normalize_total(adata_mrna, target_sum = 10000)
    sc.pp.log1p(adata_mrna)
    sc.pp.highly_variable_genes(adata_mrna,  n_top_genes=2000)
    ...
    gene_list, gene_embedding = encode_gene(mrna_data, args.data_dir + '/ATAC-RNA/gene_embedding.npz')
    mrna_data_all = mrna_data[gene_list]

    # Save preprocessed atac and protein data
    atac_data_all.to_csv(args.data_dir + '/ATAC-RNA/atac_preprocess.csv', sep=',', index=True, header=True)
    mrna_data_all.to_csv(args.data_dir + '/ATAC-RNA/rna_preprocess.csv', sep=',', index=True, header=True)
    ...
    assert atac_data_all.index.tolist() == mrna_data_all.index.tolist()
    cell_all = np.array(atac_data_all.index.values)
    print('After cells alignment: '+str(atac_data_all.shape)+' '+str(mrna_data_all.shape), flush = True)
    
    # Generate cell embedding
    cell_embedding_all, atac_data_imputed= encode_cell_all(atac_data_all, args.data_dir + '/ATAC-RNA/cell_embedding.npz')  # (3158, 100)
    ...
    query = torch.tensor(gene_embedding, dtype=torch.float32)
    ...
    df_prediction_all = pd.DataFrame(prediction_all, columns=list(gene_list), index = list(cell_pre)).T
    df_prediction_all = df_prediction_all.fillna(value=0)
    df_prediction_all.to_csv(args.out_dir + '/prediction.csv')
    df_truthvalue_all = pd.DataFrame(truthvalue_all, columns=list(gene_list), index = list(cell_pre)).T
